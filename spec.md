# kbsmash — Terminal Arcade Engine
...
Gamepad support is opt-in and optional (requires the `inputs` library).
...
```python
start(60, 24, input="pynput", gamepad=True)
...
A background thread polls USB HID events via the `inputs` library.
`update_keys()` snapshots edge-triggered presses for the frame. Hot-plugging
is supported — the polling loop retries if no device is present, so a
gamepad connected mid-game just starts working.
...
When `gamepad=False` (default) or the `inputs` library is missing, all
gamepad queries return safe defaults (`False`, empty set, `(0, 0)`, `0.0`)
so games can call them unconditionally without crashing.
